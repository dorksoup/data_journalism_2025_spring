---
title: "lab_01"
author: "Derek Willis"
date: "2025-01-16"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
# Turn off scientific notation
options(scipen=999)
```

## About this lab

To complete this lab, you need to: \* run existing code as directed (look for **Task**). \* modify existing code as directed (look for **Task**). \* write code in empty codeblocks provided to answer questions included (look for **Q**). \* write out the answer in the form of a complete sentence in the space given (look for **A**). When you are finished, commit changes and push to your personal GitHub repo, then submit the URL to this document on ELMS.

Remember to add any new verbs or concepts to your class reference notebook!

## Load libraries and establish settings

**Task** Run the codeblock below to load the Tidyverse. To run the codeblock, hit the little green play button in the upper right corner of the gray area (the codeblock) below.

```{r}
# Load the tidyverse. If you have not installed the tidyverse already, remove the # from the next line and run it first.  
# install.packages('tidyverse')
library(tidyverse)
```

## Load Data

We're going to load the UMD courses data.

**Task** Run the codeblock below to load the data, which is contained in an RDS file.

```{r}
umd_courses <- read_rds("lab_01.rds")
```

## Answer questions

**Q1** Which department has the highest mean (average) number of seats for its classes? You need to show the department with the highest average first. Which one is the most surprising/interesting to you? **A1** The Computer, Math, and Natural Sciences has the highest mean average seats. Not shocked its a big department 

```{r}
umd_courses |>
  group_by(department) |> #i am grouping by department#
  summarise(
    mean_seats = mean(seats) #making the variable "mean_seats" which involves the attribute "seats", self explanatory 

  ) |>
  arrange(desc(mean_seats)) #remember that the function for arrange uses the variable you create   
```

**Q2** Find out the greatest number of times any individual class has been offered in our dataset. Your code must show the greatest number of classes first.
**A2** AASP100 had 29 classes offered 

```{r}
umd_courses |>
  group_by(id) |> 
  summarise(
    class_count = n() #this is how to count the number of all the data ,,, within the group by (if under paranteses)
  ) |>
    arrange(desc(class_count))
  
```
```{r}
umd_courses_counted <- umd_courses |>
  group_by(id) |> 
  summarise(
    class_count = n() #this is how to count the number of all the data ,,, within the group by (if under paranteses)
  ) |>
    arrange(desc(class_count))

umd_courses_counted
  
```
**Q3** Count the number of courses for each instructor and department. Which instructor has taught the most?
**A3** Geographical Sciences	Keith Yearwood because that instructor is the highest class number that is named 

```{r}
umd_courses |>
  group_by(department, instructors) |>
  summarise(
    count_courses = n()
  ) |>
   arrange(desc(count_courses))
  
```

**Q4** Given the results for Q3, what would have made it easier to find the answer? You don't have to know *how* to accomplish this; I want you to describe in words what would make the answer easier to see. How confident are you that the answer you got is the right one, and why? **A4** Since a lot of the courses had N/A instructors, it would've been easier to filter out any of the rows that had the attribute of  "N/A" for instructors. I had to scroll a lot just to find the answer. 

**Q5** Take one of the questions you listed in the final question of the pre_lab and try to answer it here in code. If you don't know how to fully answer it, write what code you can and describe in words what else you would need to do. If you got an answer, is it interesting? **A5**
"What would be the department with the most classes? How can I do code for that." --> I got the answer business and management... which isnt interesting because I already had the answer when looking for total seats. 
```{r}
umd_courses |>
  group_by(department) |>
  summarise(
    count_courses = n()
  ) |>
   arrange(desc(count_courses))
```
