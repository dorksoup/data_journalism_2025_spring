...
baltimore_overdose <- read_csv("data/baltimore_911.csv") |>
  mutate(across(zip_code, as.character)) #zips are chracter not numbers!!
 #okay so... zcta demographics 
demographics <- read_csv("data/maryland_zcta.csv") |>
  clean_names() |>
  rename(zip_code = zcta5n)|>
  mutate(across(zip_code,as.character))

baltimore_overdose_demogr <-left_join(baltimore_overdose, demographics)
```
...
A2.
Most of the tip months for each countyis during september. But 
The location with the most calls is 314 Grove Neck RD Earleville, MD.. which is a rehab center in Cecil. As for the most 
...
#finding total number of calls per month 
...
```{r}
balt_18_over <- baltimore_overdose_demogr |>
...
balt_18_over |>
  mutate(
    callsper_capita = calls/population * 1000
  ) |>
  arrange(desc(callsper_capita))

baltimore_overdose_demogr |>
...
A4: What is the percentage of calls from the top native populated zip codes 

```{r}
baltimore_overdose_demogr |>
  group_by(zip_code) |>
  summarise(avg_native= mean(pct_native)) |>
  arrange(desc(avg_native))

# finding the percentage of each zipcode given from last code 
 baltimore_overdose_demogr|>
