...
**Task** Load some [Maryland state government payments data](https://opendata.maryland.gov/Budget/State-of-Maryland-Payments-Data-FY2008-to-FY2024/7syw-q4cy) by running the following code. We'll use the 
guess_max() 
function as an argument to use the first 10 rows to set the data type. What does the first line of the red Warning message that prints out when you load the data say? What happens when you follow its instructions? Answer below. **Answer** The warning is One or more parsing issues, call `problems () on your data frame... so i should focus on the problems function. After I use that function, the output is a a table of all the parsing problems 

```{r}
payments <- read_csv("data/State_of_Maryland_Payments_Data__FY2008_to_FY2024_20250115.csv", guess_max=10)

problems(payments)  #my edits
```

### Task 3: Reload data

**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer** No errors its just not showing the columns.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer** The vendor name field is a chracter data type. The fiscal year is under the data type of dbl or numbers. The date is under character data type 
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think explains those records? **Answer** 553 zipcodes are under five chracters length  (i used the row number to conclude this). Maybe there under the same state but how would you know
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer** The smallest loan amount is in fiscal year 2009 with a amount of 60 dollars. There is another amount that is under 100 dollars, which can be an outlier in a data set of more than 19k rows. 
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **Answer** the headers are lower space, and have underscores replacing the spaces.
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **Answer** cleaned_md_grants_loans <- md_grants_loans |> line is using the new clean grante data set, the second line is using the clean names function, third line renames the grantor header to source, the fourth line makes new data column that makes all the  data under the said headers be upper case 
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer** many 58
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer** the new data frame has 19,453 rows 
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer** The zip5 only has results of 5L type chracters, but theres some NA
