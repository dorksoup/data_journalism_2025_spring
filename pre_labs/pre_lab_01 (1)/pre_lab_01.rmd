...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer** chr and dbl (i assume quantitative and qualitative)
...
In the space below, write the name of the business from the first row of the "name" column. **Answer** ? theres no name column
...
**Answer** the first line looks into the overall data set of umd_courses, then the second line focuses on a specif group within the data set, in this case terms. The next line indicated to  summerizes the data within the group by assorting and making short cuts. Then the last line is the data being summarized, which is the class count. It makes all the objects in count_class into a set and then aligns them within the terms
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer** the summary of classes count for each term is listed in decending order
...
**Answer** the group_by( ) code now contains two varaibles, term and department. And those show the summary of class count within both of variablles
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer** The codes both show the amount of seats in each term, but the last code block gives the output of the mean, median, and sum total of seats  as its own column. TH e frist three columns are the same  
...
f <- function(r) {
  left_side <- 35 * (1 - (1 + r)^-24) / r + 1000 / (1 + r)^24
  return(left_side - 1250)
}

# Use uniroot to find the root
result <- uniroot(f, c(0.0001, 0.1))  # Adjust the range as needed
interest_rate <- result$root
interest_rate
```

Compared to the previous codeblock, what has changed and how did that affect the result? How is the data sorted?

**Answer** There is an extra two columns that show the minimum and maximum seats of each department. The columns of the min and max are after the rest of the columns. Allthough since the arrangment order is decending from the total seats column, the new min / max columns are not in their own arranged order
...
1.  If majority of the classes have a minimum of zero seats... do they just get cancelled if no one signs up?

2.  Why does the Business and management  department have the most seats total for classes?

3.  What would be the department with the most classes? How can I do code for that. 
