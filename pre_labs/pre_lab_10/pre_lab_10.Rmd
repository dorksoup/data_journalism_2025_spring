...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer**  Now its is the Objext id, full postal Zipcode, zip name , existing ?, zipcode again but short, some type of cordinate and another and the geometry 
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** ow its is the Objext id, full postal Zipcode, zip name , existing ?, zipcode again but short, some type of cordinate and another and the geometry of each location in maryland
...
**Task** Run the following code. Describe the output in the space below. **Answer** its a map or maryland with zipcode regions divided/. The map also has a grid with degree breaks 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Now it is 19 colums about information about the counties, might be all the us counties 
...
**Task** Run the following code. Describe the output in the space below. **Answer**  now its the ifnormation of maryland states, because maryland is stae 24 on the census
...
**Task** Run the following code. Describe the output in the space below. **Answer** Now it is the Maryland counties region boundaries on the map. Still agrid 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The zipcode map is over layed on the county map
...
**Task** Run the following code. Describe the output in the space below. **Answer** the xipcodes and each foreclosure in march
...
**Task** Run the following code. Describe the output in the space below. **Answer** Now the zipcode date frame attached the march foreclosure columns. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Now each zipcode region in the map is color coded by the amount of forclosures in march.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The county notices data has the Date, Type, County, number of Notices. The population data is the zipcodes and counties and the population number. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** the population dataframe now has a geometry column with the coordinates. the multipolygon 
...
**Task** Run the following code. Describe the output in the space below. **Answer** county names are in title case but they are under the column "name" and it has Maryland attached to it 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The population data is now clean. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** population number and the forclosure notices are in the same data set 
...
**Task** Run the following code. Describe the output in the space below. **Answer** there is now a column for the percentage rate of notices in reference to the poplulation 
...
**Task** Run the following code. Describe the output in the space below. **Answer** There is a choropelth map with color of each county in accordance to the number of notices per 1k population (the last column we mutated into the data frame)
...
**Task** Run the following code. Describe the output in the space below. **Answer** higher contrast of color
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** i dont really see a pattern ... 
