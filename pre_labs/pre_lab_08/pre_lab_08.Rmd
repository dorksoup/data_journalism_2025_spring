...
1 line is making a new data set with the data frame we have before.
2 line is mutating the data set to change the month column data type to be months, labeling it as each name of the month
3 line is grouping the next following data by month column, 
4 line is making a column of the total calls and summrizing the number in the next column,
 5 line is arrangin gthe data set in descending order by the number of total calls. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** theres nothing just blank space 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** now there is a different bars that corrspond o each month, labels for the months (x axis) and labels for the count of calls (y axis)
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** the x= reorder (month, total_calls) reordered the x values to go in an order that is chronolgical of the total calls (y values)
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** coord_flip flips the coordinates so the y values are now the x axis and vice versa 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** minimal theme so there isnt a grey fill in behind the bars 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer** the bacground is blue, maybe for accessibility 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** there is now a title for the chart, the axis have its own labels and it has a caption 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** its a data frame of the dates of calls and another column of the total calls for each date
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** there is a line chart of the number of calls of each day from the data set. Kind of hard to read. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** This chart has tried to label every singe week as date breaks on the y axis but it makes it very hard to read the info under the line, 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** there is the same date breaks but the n=dates are outputed in a nagle so they dont overlap with eachother 
