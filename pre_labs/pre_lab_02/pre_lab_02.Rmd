...
**Answer** the filter at first helps to string out specific rows that the select command can display specific columns 
...
**Answer:** The date column is now listed out in yyyy-mm-dd , and has the <date> format
...
**Answer** although it still shows the year-month-day, the mutate function lumped all the units of each month from each year into its own row. The days will be 1
...
**Answer** The code is making the variable that has columns with all the votes number combined, and then a percentage of moore voters within those total votes. The mutate function helps with that 
...
**Answer** The percentage moore column now is in descending order which is easier to follow/ 
...
**Answer** you get to see the counties with the least of votes for moore so it can be intrestign if youre reporting somehting about them. 
...
**Answer** str_to_lower() converts to lower case. str_to_title() converts to title case, where only the first letter of each word is capitalized. str_to_sentence() convert to sentence case, where only the first letter of sentence is capitalized.
...
**Answer** the mutate function is creating more variables about other states expenses with Maryland. the string detects address that have what ever input is typed within the "" and then will display it with the state initals 
...
**Answer** It makes sense but I am not sure if I can comprehend it as well. The titles are confusing so I am not sure what information the data set is telling me. But i understood how the code works
...
1. How can I produce code that would select each individual professor within the journalism department, and their courses?

2.  For the umd courses, what code could be written to display only the classes from a certain term, using dates.

3.  What code could show the maryladn expenses with each state, but only select the fund type. `
