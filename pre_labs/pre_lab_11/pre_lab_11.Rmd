...
There is the file id name for each of the alerts, 499 rows. THE URL The doctor info: id The Names of them, the type of alert and the year and  date with mutlple variations anad the text
```{r}
...
It only list each unique word once now, but the dog and Dog are treated as their own independent words
```{r}
...
**Task** Run the following code and describe the output of using the `unnest_tokens` function. **Answer** It broke apart all the words in the 'text' column and put it in a list called 'word' , but each word is lowercase with no punctuation 
...
**Task** Run the following code and describe what the resulting graphic shows. Is it interesting? **Answer** Its not really intresting. Becuase the most common word is a word thats not a noun, verb, adverb or adjective. 
...
**Task** Run the following code and describe the results. Is it more interesting than before? **Answer** The names are much more interesting, because they either describe something, its a name of something, or its an action. Plus there is a tally and percent column to see a better picture 
...
**Task** Run the following code and describe the results. **Answer** License was the most used word in 2023 but in 2022 it was top 4
...
**Task** Run the following code and describe the results. **Answer** The results before 2022, n grams helps show the  combonation of two word phrases broken up into a list. 

```{r}
alerts |>
  filter(date < '2022-01-01') |> #edited for the dat bc it said 2024
...
**Task** Run the following code and describe the results. **Answer**  Almost the same words, but Ms cook is shown in the top 10... and moral turpitude, rehabilitation agreement 

```{r}
alerts |>
  filter(date > '2022-12-31' & date < '2024-01-01') |> #edited for the date again 
  unnest_tokens(bigram, text, token = "ngrams", n = 2) |>
  separate(bigram, c("word1", "word2"), sep = " ") |>
  filter(!word1 %in% stop_words$word) |>
  filter(!word2 %in% stop_words$word) |>
...
**Task** Run the following code and describe the results. **Answer** Individual 10 and 4 are standouts 
...
**Task** Run the following code and describe the results. Do any of the sentiments seem incorrect or counter-intuitive? **Answer** "fine" is seen as positive, but it can also be a finacial meaning, which may not be positive. 
